...
- Backend tests pass cleanly without Jest `forceExit`
- Frontend tests pass normally and under coverage
...
- Backend Jest no longer relies on `forceExit`; test imports do not start cron schedulers or optional runtime background services in `NODE_ENV=test`.
- Frontend coverage runs are slower than normal test runs; the long App and Agent Management tests now use timeouts that are compatible with coverage instrumentation.
