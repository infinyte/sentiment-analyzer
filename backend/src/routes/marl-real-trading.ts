...
export function createMarlRealTradingRouter(brokerRepo: IBrokerRepository): Router {
  const router = Router();

  // ─── POST /api/marl/broker/credentials ──────────────────────────────────────
  // Store encrypted broker credentials. Never returns the raw key/secret.

  router.post('/api/marl/broker/credentials', async (req: Request, res: Response) => {
    ...
    try {
      const id = randomUUID();
      await brokerRepo.saveCredential({
        id,
  ...
  router.get('/api/marl/broker/credentials', async (req: Request, res: Response) => {
    if (!requireApiKey(req, res)) return;
    try {
      const raw = await brokerRepo.listCredentials();
      const creds = raw.map(c => ({
        id:        c.id,
        label:     c.label,
        provider:  c.provider,
        mode:      c.mode,
  ...
  router.get('/api/marl/broker/credentials/picker', async (_req: Request, res: Response) => {
    try {
      const raw = await brokerRepo.listCredentials();
      const creds = raw.map(c => ({
        id:       c.id,
  ...
  router.delete('/api/marl/broker/credentials/:id', async (req: Request, res: Response) => {
    ...
    const deleted = await brokerRepo.deleteCredential(id);
  ...
  router.post('/api/marl/broker/connect/:id', async (req: Request, res: Response) => {
    ...
    try {
      const cred = await brokerRepo.getDecryptedCredential(id);
      if (!cred) return res.status(404).json({ error: 'Credential not found' });

      const adapter = createBrokerAdapter(cred);
      await brokerRegistry.register(adapter);

      logger.info('broker adapter connected', { id, provider: cred.provider, mode: cred.mode });
      res.json({ connected: true, id, provider: cred.provider, mode: cred.mode });
    } catch (err) {
  ...
  router.get('/api/marl/broker/connected', async (req: Request, res: Response) => {
    if (!requireApiKey(req, res)) return;
    try {
      const ids = brokerRegistry.listIds();
      const creds = await brokerRepo.listCredentials();
  ...
  router.get('/api/marl/broker/orders/:competitionId', async (req: Request, res: Response) => {
    if (!requireApiKey(req, res)) return;
    const { competitionId } = req.params;
    const agentId = req.query.agentId as string | undefined;

    try {
      const raw = await brokerRepo.getOrders(competitionId, agentId);
